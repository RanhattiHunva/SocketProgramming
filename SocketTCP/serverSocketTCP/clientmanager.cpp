...
void client_list::addElement(const char* inputIP, int inputNum)
{
...
int client_list::findElement(int inputNum, client_information& contain_information)
{
    std::lock_guard<std::mutex> guard(client_mutext);
...
int client_list::removeElement(int inputNum)
{
    std::lock_guard<std::mutex> guard(client_mutext);
    for (unsigned long i=0; i< client_list.size(); i++)
    {
...
unsigned long client_list::getSize()
{
    std::lock_guard<std::mutex> guard(client_mutext);
